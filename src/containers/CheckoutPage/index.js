...
export const CheckoutPage = (props) => {
  ...
  const onConfirmOrder = () => {
    ...
    const items = Object.keys(cart.cartItems).map((key) => ({
      ...
      discountOnQuantity: cart.cartItems[key].discountOnQuantity,
      discountPercentage: cart.cartItems[key].discountPercentage,
      sellerId: cart.cartItems[key].sellerId,
      status: 'pending'
    }));
    const payload = {
  ...
  useEffect(() => {
    if (confirmOrder && user.placedOrderId) {
      history(`/order_details/${user.placedOrderId}`);
    }
  }, [confirmOrder, history, user.placedOrderId]);
